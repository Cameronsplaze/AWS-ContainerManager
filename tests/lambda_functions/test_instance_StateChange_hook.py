...
class TestInstanceStateChangeHook:
    @classmethod
    def setup_class(cls):
        os.environ["HOSTED_ZONE_ID"] = "Z_DUMMY_12345"  # Dummy value
        os.environ["DOMAIN_NAME"] = "test.example.com"
        os.environ["UNAVAILABLE_IP"] = "0.0.0.0"
        os.environ["DNS_TTL"] = "1"
    ...
    def teardown_class(cls):
        ...
        del os.environ["UNAVAILABLE_IP"]
        del os.environ["DNS_TTL"]
        del os.environ["RECORD_TYPE"]
    ...
    def setup_method(self, _method):
        ...
        os.environ["HOSTED_ZONE_ID"] = hosted_zone["HostedZone"]["Id"].split("/")[-1]  # Update the env var to the real ID
        ...
        self.instance_StateChange_hook.route53_client.change_resource_record_sets(
            ...
            ChangeBatch={
                'Changes': [{
                    ...
                    'ResourceRecords': [{'Value': os.environ['UNAVAILABLE_IP']}],
    ...
    def test_starting_record_values(self):
        ...
        a_record = records[2]
        assert a_record["Name"] == os.environ["DOMAIN_NAME"] + "."
        assert a_record["Type"] == os.environ["RECORD_TYPE"]
        assert a_record["TTL"] == int(os.environ["DNS_TTL"])
        assert a_record["ResourceRecords"] == [{"Value": os.environ["UNAVAILABLE_IP"]}]
    ...
    def test_lambda_sets_ip_events(self, event_type, desired_capacity, monkeypatch):
    ...
    def test_lambda_exit_if_asg_instance_coming_up_on_terminate(self, monkeypatch):
        """
    ...
    def test_lambda_raises_on_unknown_event(self):
